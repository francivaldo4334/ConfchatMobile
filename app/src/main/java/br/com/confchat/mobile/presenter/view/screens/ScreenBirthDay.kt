...
@Composable
fun ScreenBirthDay(navController: NavController, AuthDoc: AuthDoc, viewModel: AuthViewModel = hiltViewModel()) {
    ...
    LaunchedEffect(key1 = Unit, block = {
        birthDay = AuthDoc.register.birthDay
    })
    LazyColumn(
        modifier = Modifier
            .fillMaxSize()
        ...
        item {
            if(isLoad){
                ...
                ComponentButton1(
                    ...
                    enabled = MyValidUtil().validDate(birthDay)
                ) {
                    AuthDoc.register.birthDay = birthDay
                    isLoad = true
                    viewModel.register(AuthDoc){isSuccess,messge ->
                        if(isSuccess){
...
private fun ScreenBirthDayPreview() {
    ConfchatTheme {
        ScreenBirthDay(rememberNavController(),AuthDoc())
    }
}